...
(function($){
	...
	var STATE_OPEN = 0,
		STATE_COLLAPSED = 1,
		STATE_DISMISSED = 2;
	// var STATE_PROPERTY = 'donorbanner-state';
	// var STATE_PROPERTY = 'fourth2014-state';
	var STATE_PROPERTY = 'sotu2015-state';
	...
	Banner.prototype.saveState = function() {
		if (window.localStorage) {
			localStorage.setItem(STATE_PROPERTY, this.state);
		}
	};
	...
	Banner.prototype.loadState = function() {
		...
		if (window.localStorage) {
			var storedState = localStorage.getItem(STATE_PROPERTY);
			if (storedState !== null) {
